...
def affine_registration_dwi(fixed_image, moving_image, atlas, plot=False):
...
for mri_volume in files:
    ...
    dwi_path = mri_volume.split('\\')[0] + ('\\') + mri_volume.split('\\')[1] + '/ADC.nii.gz'
    print('---------------------------------------------------------')
    ...
    m_res, atlas_res, dice = affine_registration_dwi(fixed_image, moving_image, atlas)

    dwi = sitk.ReadImage(dwi_path)
    dwi_rescaled = rescale_voxel_size(dwi)
    diffusion_map = resample_img(dwi_rescaled, new_spacing=template.GetSpacing(), interpolator=sitk.sitkLinear)
    ...
    sitk.WriteImage(diffusion_map, output_string + '_diffusion.nii')
