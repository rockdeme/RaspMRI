...
volume_path = glob.glob('G:\mri-dataset\diffusion_0h\*\ADC.nii.gz')

volume_array = sitk.ReadImage(volume_path[0])
volume_array = sitk.GetArrayFromImage(volume_array)

plt.hist(volume_array[volume_array != 0].flatten(), bins=300)
plt.show()
